...
pub(crate) const VIDIOC_S_FMT: u64 = _iowr(V4L2_TYPE, 5, V4L2_FORMAT_SIZE);
pub(crate) const VIDIOC_G_FMT: u64 = _iowr(V4L2_TYPE, 4, V4L2_FORMAT_SIZE);
pub(crate) const VIDIOC_REQBUFS: u64 = _iowr(V4L2_TYPE, 8, V4L2_REQUESTBUFFERS_SIZE);
pub(crate) const VIDIOC_QUERYBUF: u64 = _iowr(V4L2_TYPE, 9, V4L2_BUFFER_SIZE);
pub(crate) const VIDIOC_QBUF: u64 = _iowr(V4L2_TYPE, 15, V4L2_BUFFER_SIZE);
pub(crate) const VIDIOC_DQBUF: u64 = _iowr(V4L2_TYPE, 17, V4L2_BUFFER_SIZE);
pub(crate) const VIDIOC_STREAMON: u64 = _iow(V4L2_TYPE, 18, 4);
pub(crate) const VIDIOC_STREAMOFF: u64 = _iow(V4L2_TYPE, 19, 4);
pub(crate) const VIDIOC_S_CTRL: u64 = _iowr(V4L2_TYPE, 28, V4L2_CONTROL_SIZE);
...
pub(crate) fn ioctl_s_fmt(fd: RawFd, fmt: &mut v4l2_format) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_S_FMT, fmt as *mut _) };
    if ret < 0 {
...
pub(crate) fn ioctl_g_fmt(fd: RawFd, fmt: &mut v4l2_format) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_G_FMT, fmt as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_reqbufs(fd: RawFd, req: &mut v4l2_requestbuffers) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_REQBUFS, req as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_querybuf(fd: RawFd, buf: &mut v4l2_buffer) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_QUERYBUF, buf as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_qbuf(fd: RawFd, buf: &mut v4l2_buffer) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_QBUF, buf as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
            request: "VIDIOC_QBUF",
            source: std::io::Error::last_os_error(),
...
pub(crate) fn ioctl_dqbuf(fd: RawFd, buf: &mut v4l2_buffer) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_DQBUF, buf as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_streamon(fd: RawFd, buf_type: u32) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_STREAMON, &buf_type as *const _) };
    if ret < 0 {
...
pub(crate) fn ioctl_streamoff(fd: RawFd, buf_type: u32) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_STREAMOFF, &buf_type as *const _) };
    if ret < 0 {
...
pub(crate) fn ioctl_s_ctrl(fd: RawFd, ctrl: &v4l2_control) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_S_CTRL, ctrl as *const _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_expbuf(fd: RawFd, expbuf: &mut v4l2_exportbuffer) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_EXPBUF, expbuf as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_subscribe_event(
    ...
    let ret = unsafe { libc::ioctl(fd, VIDIOC_SUBSCRIBE_EVENT, sub as *const _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
...
pub(crate) fn ioctl_dqevent(fd: RawFd, event: &mut v4l2_event) -> crate::error::Result<()> {
    let ret = unsafe { libc::ioctl(fd, VIDIOC_DQEVENT, event as *mut _) };
    if ret < 0 {
        return Err(crate::error::Error::Ioctl {
