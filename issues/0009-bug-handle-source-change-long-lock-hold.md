...
- `H264Decoder::drop`: `poller.stop()` が poller スレッドの join でフェーズ 2 の完了を待つため、ioctl 実行中に fd が close されることはない。`capture_started` が false の間は CAPTURE STREAMOFF を発行しない（0004 の設計どおり）
